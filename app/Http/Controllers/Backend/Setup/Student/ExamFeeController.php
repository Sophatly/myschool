...
class ExamFeeController extends Controller
{
    ...
    public function ExamFeeClassData(Request $request){
        ...
        foreach ($allStudent as $key => $v) {
            ...
            $registrationfee = FeeCategroyAmount::where('fee_category_id','3')->where('class_id',$v->class_id)->first();
            $color = 'success';
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.exam.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.' &exam_type_id='.$request->exam_type_id.'">Exam Fee Payslip</a>';
            $html[$key]['tdsource'] .= '</td>';
    ...
    public function ExamFeePayslip(Request $request){
        ...
        $studentName = User::find($student_id);
        $getName = $studentName->name;
        $finalName = $getName. 'Exam Fee';
