...
class MonthlyFeeController extends Controller
{
    ...
    public function MonthlyFeeClassData(Request $request){
        ...
        foreach ($allStudent as $key => $v) {
            ...
            $registrationfee = FeeCategroyAmount::where('fee_category_id','2')->where('class_id',$v->class_id)->first();
            $color = 'success';
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.monthly.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.' &month='.$request->month.'">Monthly Payslip</a>';
            $html[$key]['tdsource'] .= '</td>';
    ...
    public function MonthlyFeePayslip(Request $request){
        ...
        $finalName = $getName.$request->month;
